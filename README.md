...
* 2025-02: We released [s1.1](https://huggingface.co/oof-baroomf/s1.1-32B) a better model than s1 by reusing the same s1K questions but with reasoning traces generated by r1 instead of Gemini: [s1K-1.1](https://huggingface.co/datasets/oof-baroomf/s1K-1.1). Check [this tweet](https://x.com/Muennighoff/status/1889310803746246694) for details
...
- **Model**: https://hf.co/oof-baroomf/s1-32B
- **Data**: https://hf.co/datasets/oof-baroomf/s1K
    - s1-prob: https://hf.co/datasets/oof-baroomf/s1-prob
    - s1-teasers: https://hf.co/datasets/oof-baroomf/s1-teasers
    - Full 59K: https://hf.co/datasets/oof-baroomf/data_ablation_full59K
...
model = LLM(
    "oof-baroomf/s1.1-32B",
    tensor_parallel_size=2,
)
tok = AutoTokenizer.from_pretrained("oof-baroomf/s1-32B")
...
model = LLM(
    "oof-baroomf/s1-32B", # s1 originally gets this prompt wrong but with budget forcing it fixes it
    tensor_parallel_size=2,
)
...
model_name = "oof-baroomf/s1.1-32B"
...
To train s1-32B/s1.1-32B, we recommend 16 H100 GPUs i.e. 2 nodes with 8 each. For s1.1, we set the block size to 20000 to avoid OOM (https://github.com/oof-baroomf/s1/blob/0ad4b3de32507b4aa0d4be28f336276ee99b2315/train/sft.sh#L17); Check the wandb logs [here](https://wandb.ai/hashimoto-group/o1/runs/m1ilia77/overview).

Quick start:
```
git clone https://github.com/oof-baroomf/s1.git
cd s1
...
All our evaluation result files are at: https://hf.co/datasets/oof-baroomf/results

To run REBASE: commands are in `eval/rebase/run.sh`
Note that for the evaluations in the Discussion section with REBASE we used https://huggingface.co/oof-baroomf/step-conditional-control-old trained on an older version of our dataset https://huggingface.co/datasets/oof-baroomf/s1K-step-conditional-control-old and run on an older version of our evaluation using https://huggingface.co/datasets/Maxwell-Jia/AIME_2024.

### Data

To recreate s1K follow the steps below. In various files you will have to rename the organizations `oof-baroomf` and `qfq` with an organization that you own. **Note that [s1K-1.1](https://huggingface.co/datasets/oof-baroomf/s1K-1.1) is a better dataset generated with r1 traces instead of Gemini traces.**
